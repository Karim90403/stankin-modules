...
const Login = async () => {
  try {
    const response = await axios.post("/api/autorisation", { login: userLogin.value, password: userPassword.value })
...
<template>
  <section>
    ...
    <form class="w-full h-full mt-8 space-y-6 flex flex-col justify-center items-center" action="#" method="POST" @submit.prevent="Login()">
